...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/penilaian/kriteria/updateutama",
		data:$(this).serialize(),
		error:function(){
...
{	
	for(i=1;i<=<?=$jumlah;?>;i++)
	{
		...
		$("#pri-b"+i).val(totprio);		
	    $.ajax({url: "<?=base_url('admin/penilaian/kriteria/setTempPrior/'.$penilaianid)?>/"+i+"/"+totprio, success: function(result){/*result*/}});		
	}
}
...
function showsubkriteria()
{
	$.ajax({
			type:'get',
			dataType:'html',
			url:"<?=base_url(akses().'/penilaian/kriteria/getsubcontainer');?>",
			data:"penilaianid=<?=$penilaianid;?>",
			error:function(){
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_kriteria($penilaianid,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
