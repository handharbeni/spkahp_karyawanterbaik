...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/penilaian/kriteria/updatesub",
		data:$(this).serialize(),
		error:function(){
...
$("#prioform").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/penilaian/kriteria/updatesubprioritas",
		data:$(this).serialize(),
		error:function(){
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_subkriteria($penilaianid,$kriteriaid,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
